...
```bash
RUN_DIR=$(bash ~/ei-claude-plugin/skills/spawn-agent-team/scripts/setup-run-dir.sh cache-strategy)
echo "$RUN_DIR"
# /Users/damon/Claude-Stuff/spawn-team-runs/2026-05-04-cache-strategy-143022
```
...
```
Write your output to: /Users/damon/Claude-Stuff/spawn-team-runs/2026-05-04-cache-strategy-143022/researchers/researcher-1.md
Return when done.
```

If the run-dir variable contains `~`, expand it before substituting into the prompt. Agents should never see `~/Claude-Stuff/...`; they should see `/Users/damon/Claude-Stuff/...`.
...
1. The lead's final synthesis is the canonical output. Damon reads it; downstream skills consume it.
...
Default: keep all run dirs. Damon decides when to archive.
